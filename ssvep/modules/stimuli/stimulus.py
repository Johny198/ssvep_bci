...
class Square():
    ...
    def op_change(self, freq, phase, wave, frame):  # opacity modulation by waveform
        if wave == 'sin':
            self.sq_stim.opacity = 0.5 * (1 + np.sin(2 * np.pi * freq
                                          * (frame/60) + np.pi*phase))
        elif wave == 'square':
            self.sq_stim.opacity = 0.5 * (1 + signal.square(2 * np.pi * freq
                                          * (frame/60) + np.pi * phase))
        elif wave == 'sawtooth':
            self.sq_stim.opacity = 0.5 * (1 - signal.sawtooth(2 * np.pi * freq
                                          * (frame / 60) - np.pi * phase))
        self.sq_stim.draw()
